...
- **DSL Gradle** : **Groovy DSL** (`build.gradle`, `settings.gradle`) — choix projet acté. Ne pas migrer vers Kotlin DSL (`*.kts`) sauf demande explicite.
...
**Package root** : `com.ntech.cabosse` — canonique pour tout le code Java backend (matche le `group` Gradle). Toute autre racine (`tech.neiba.cabosse`, `com.neiba.cabosse`) est interdite.

Organisation par **feature**, pas par couche technique :

```
com.ntech.cabosse.purchase
  ├── controller
  ├── service
  ├── repository
  ├── entity
...
**Interdit** : `com.ntech.cabosse.controllers.purchase`, `com.ntech.cabosse.services.purchase`. Le couplage est de proximité métier, pas de proximité technique.
...
Toutes ces classes vivent sous `com.ntech.cabosse.shared.*`. Voir le document `shared-constants.md` (NEIBA-ARCH-2026-002) pour l'inventaire complet et les conventions.
...
### 6.4 Fichiers uploadés — pas de binaire dans les entités

Aucun champ `byte[]` ou `Binary` ne descend dans une entité métier. Tout fichier uploadé (logo, image produit, attachment, export généré, …) est représenté par une référence à un document `CloudFileEntity` qui porte les méta (`storageBackend`, `storagePath`, `mimeType`, `sizeBytes`, `type`, `ownerEntityId`, `ownerEntityType`, `publicUrl`, dates d'audit, `archivedAt`). Le binaire vit dans un backend de stockage configurable (filesystem local au MVP, S3-compatible à terme) accédé via l'abstraction `FileStorage`.

L'entité métier référence le fichier par son `UUID` (ex. `TenantBranding.logoFileId`) ; seuls `mimeType` et `sizeBytes` peuvent être dupliqués pour l'affichage rapide. Tout le reste — `originalFileName`, `storagePath`, `publicUrl`, dates — n'est jamais dupliqué.
