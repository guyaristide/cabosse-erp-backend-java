package com.ntech.cabosse.variety.controller;
...
import com.ntech.cabosse.variety.service.VarietyService;
...
@Tag(name = "Variétés", description = "Référentiel des variétés cultivées tenant")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@Authenticated
@RequiresPermission(Permission.REF_VARIETY_READ)
public class VarietyResource {

    @Inject VarietyService service;
    ...
    @POST
    @RequiresPermission(Permission.REF_VARIETY_WRITE)
    @RolesAllowed({ Roles.TENANT_ADMIN, Roles.USER, Roles.PLATFORM_ADMIN })
    public Response create(@Valid VarietyUpsertDto p) {
    ...
    public Response update(@PathParam("id") UUID id, @Valid VarietyUpsertDto p) {
